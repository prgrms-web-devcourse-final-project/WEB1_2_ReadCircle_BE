...
public interface SellerRepository extends JpaRepository<Seller, Long> {
    @Query(" SELECT s FROM Seller s Join s.book b WHERE s.userId = :userId AND b.process = :process ")
    Page<Seller> getSellerByUserId(String userId, @Param("process")BookProcess process, Pageable pageable);

    @Query(" SELECT s FROM Seller s Join s.book b WHERE b.process = :process ")
